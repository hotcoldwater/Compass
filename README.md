...
- 지원 일정과 상태 관리
- AI 초안 생성 전, 사용자가 답변 흐름(개요)을 먼저 작성하도록 하고, 해당 흐름만으로 초안 작성이 부족하면 필요한 추가 질문을 자동으로 던진 뒤 초안을 생성. 추가 질문이 필요 없으면 바로 초안 생성
- 지원동기 등 기업 연계형 문항을 위해 기업의 인재상·정책·신년사 등 기업 정보를 사용자가 입력하고 개인의 강점과 연계해 작성하는 절차 (구체적 입력·활용 방식은 추후 설계)
